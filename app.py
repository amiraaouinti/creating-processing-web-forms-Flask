...
from flask_wtf.file import FileAllowed, FileRequired # FileAllowed is a validator which checks if the type of the file is allowed
import sqlite3
import os
import datetime
...
def save_image_upload(image):
    # This will help us to make sure that every image is unique based on the datetime
    format = "%Y%m%dT%H%M%S"
    now = datetime.datetime.utcnow().strftime(format)
    random_string = token_hex(2)
    filename = random_string + "_" + now + "_" + image.data.filename
    filename = secure_filename(filename) # This will make us sure that the file name is secure
...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect("db/shop.db")
    return db
